...
export class ElevatedLinkButton<P extends LinkChild> extends React.Component<P> {
    public render() {
        const { name, href } = this.props;
        return (
            <button
                className="rounded-sm font-medium underline px-2 py-2 bg-nord1 text-nord4 hover:text-nord5 hover:bg-nord2"
            >
                <a href={href}>{name}</a>
            </button>
        );
